...
import com.zozofood.client.RestaurantClient;
import com.zozofood.client.UserClient;
...
import com.zozofood.exception.OrderNotFoundException;
...
public class OrderServiceImp implements OrderService {
	...
	public OrderServiceImp( RestaurantClient restaurantClient, OrderRepository orderRepository, UserClient userClient) {
	...
	public OrderResponseDto createOrder(OrderRequestDto orderRequestDto) {
		...
		List<OrderItems> orderItems = orderRequestDto.getOrderItems().stream().map(itemDto -> {
			...
			item.setSubtotalPrice(calculateSubtotalPrice(itemDto.getItemId(), itemDto.getQuantity()));
			return item;
		}).collect(Collectors.toList());

		order.setOrderItemList(orderItems);
		BigDecimal subtotal = orderItems.stream().map(OrderItems::getSubtotalPrice).reduce(BigDecimal.ZERO,
				BigDecimal::add);
		...
		OrderResponseDto orderResponse = orderToOrderResponse(savedOrder);
//	        sendOrderNotification(orderResponse);
	...
	public OrderResponseDto getOrderById(Long orderId) {
		...
		return ResponseEntity.status(HttpStatus.OK).body(orderToOrderResponse(getorder)).getBody();
	}

//	private BigDecimal calculateSubtotalPrice(Long itemId, int quantity) {
//		ResponseEntity<Double> price = restaurantClient.getItemPriceById(itemId);         //getPriceByMenuItemId(itemId);
//		return price.multiply(BigDecimal.valueOf(quantity));
//	}
	
	private BigDecimal calculateSubtotalPrice(Long itemId, int quantity) {
	    ResponseEntity<Double> response = restaurantClient.getItemPriceById(itemId);
	    Double priceDouble = response.getBody(); // Extract the Double value
	    if (priceDouble == null) {
	        throw new IllegalArgumentException("Price not found for itemId: " + itemId);
	    }
	    BigDecimal price = BigDecimal.valueOf(priceDouble); // Convert to BigDecimal
	    return price.multiply(BigDecimal.valueOf(quantity)); // Perform multiplication
	}

	@Override
	public Page<OrderResponseDto> getAllOrders(PageRequest pageRequest) {
		Page<OrderResponseDto> map = orderRepository.findAll(pageRequest).map(this::orderToOrderResponse);
		return map;
	}

	
	public ResponseEntity<UserDto> getOrderInfo(Integer userId) {
        ResponseEntity<UserDto> userDTO = userClient.getUserById(userId);
        return  userDTO;
    }
